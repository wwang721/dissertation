...
\def\FontPackage{newtxtext}                 % Times-like text font (was: lmodern);
                                            % matching math font newtxmath below
...
%     bm,cancel,dsfont,mathtools,mathbbol,mathrsfs,siunitx,upgreek}
\usepackage{amsmath,amsthm,autobreak,cancel,mathtools,siunitx}
%% Times-compatible math font.  newtxmath supplies \mathbb and the amssymb
%% symbols itself, so amssymb / amsfonts / mathbbol / mathrsfs / dsfont /
%% upgreek are dropped (none of their commands are used, and keeping them
%% exceeds LaTeX's 16 math-alphabet limit).  Load after amsmath; bm after it.
...
%% In the reference list we do NOT print the DOI as standalone text.
%% Instead each entry gets ONE hyperlink (to its DOI, or its URL when no DOI
%% is given): for @article the "journal volume, pages (year)" run (APS
%% style; see the article driver below), and for books and other types the
%% title (see the \nattitlelink hook further down).  \fullcite (the chapter
%% openers) is a citation, not a bibitem, so the toggle below stays false
%% there: it keeps showing the DOI as "DOI: <link>" text exactly as before.
\newtoggle{bbx@biblink}                 % true only inside the reference list
\AtEveryBibitem{\toggletrue{bbx@biblink}}
\AtEveryCite{\togglefalse{bbx@biblink}}
\DeclareFieldFormat{doi}{%              % \fullcite shows "DOI: <link>"; the
  \iftoggle{bbx@biblink}                % reference list suppresses that text
    {}%
    {DOI:\addspace
     \ifhyperref
...
%% wrap #1 in an \href to the entry's DOI (else its URL); plain if neither
\newcommand{\nathref}[1]{%
  \iffieldundef{doi}
    {\iffieldundef{url}{#1}{\href{\thefield{url}}{#1}}}
    {\href{https://doi.org/\thefield{doi}}{#1}}}
%% @article and @inproceedings link the trailing publication run:
%% "journal volume, pages (year)" for an article ...
\newbibmacro*{nat@jouryear}{%
...
\newbibmacro*{nat@jouryear+link}{%
  \printtext{\relax}% flush the pending "." (after the title) OUTSIDE the link
  \nathref{\usebibmacro{nat@jouryear}}}% (\printtext{} alone is a no-op)
%% ... and "booktitle volume (year), pages" for a proceedings paper
\newbibmacro*{nat@procrun}{%
...
\newbibmacro*{nat@procrun+link}{%
  \printtext{\relax}% keep the leading "In" / punctuation outside the link
  \nathref{\usebibmacro{nat@procrun}}}
%% article driver: identical to nature.bbx, except that the journal->year
%% block is emitted as one hyperlink in the reference list (bbx@biblink
%% true); \fullcite (bbx@biblink false) keeps the original layout with the
%% visible "DOI: <link>".
\DeclareBibliographyDriver{article}{%
  ...
  \printfield{version}%
  \newunit\newblock
  \iftoggle{bbx@biblink}
    {\usebibmacro{nat@jouryear+link}}
    {\usebibmacro{journal+issuetitle}%
     \newunit
     \usebibmacro{byeditor+others}%
     \newunit
...
%% inproceedings driver: article-like.  Identical to nature.bbx, except the
%% "booktitle volume (year), pages" run becomes one hyperlink in the
%% reference list (bbx@biblink true) while the paper title is NOT linked;
%% \fullcite (bbx@biblink false) keeps the original layout incl. "DOI: <link>".
\DeclareBibliographyDriver{inproceedings}{%
  ...
  \usebibmacro{in:}%
  \iftoggle{bbx@biblink}
    {\usebibmacro{nat@procrun+link}}
    {\usebibmacro{nat@procrun}}%
  \newunit\newblock
...
\usepackage{CJKutf8}                        % add Chinese characters
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% END LaTeX PACKAGES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%% HYPERREF PACKAGE
\definecolor{dark_blue}{rgb}{0.176,0.184,0.569}
\def\HyperlinkColor{dark_blue}      % color for hyperlinks
...
%% footmisc's hang/flushmargin options redefine \@makefntext and drop
%% hyperref's Hfootnote.N anchor, so footnote-mark links land at the
%% top of the document. Re-inject the anchor inside \@makefntext.
\makeatletter
...
%% in the reference list, hyperlink the title of every NON-article entry
%% (books etc.) to its DOI (else URL); articles link their journal run
%% instead, and \fullcite / entries without a DOI or URL are left plain.
\newcommand{\nattitlelink}[1]{%
  \ifboolexpr{ togl {bbx@biblink}
               and not test {\ifentrytype{article}}
               and not test {\ifentrytype{inproceedings}}
               and ( not test {\iffieldundef{doi}}
                     or not test {\iffieldundef{url}} ) }
    {\nathref{\printtext[title]{#1}}}%
    {\printtext[title]{#1}}}
\renewbibmacro*{title}{%
...
%% in the reference list, hyperlink the booktitle of @inbook / @incollection
%% entries (whose own chapter title is not printed by this style) to the DOI.
\DeclareFieldFormat{booktitle}{%
  \ifboolexpr{ togl {bbx@biblink}
               and ( test {\ifentrytype{inbook}}
...
\setlength{\cftchapnumwidth}{\widthof{\normalfont\textbf{Chapter~XX}}}
...
\fancyhead[L]{\large\nouppercase \leftmark} % enlarged running header, Abstract only
\include{02-abstract}
\fancyhead[L]{\nouppercase \leftmark}       % restore normal header for the rest


%%%% OPTIONAL but COMMON: preface chapters
\include{03-dedication}
\fancyhead[L]{\large\nouppercase \leftmark} % enlarged running header, Acknowledgement only
...
% \include{99-appendix-A}
% \include{99-appendix-B}
