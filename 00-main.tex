...
%% Instead, for @article entries the "journal volume, pages (year)" run is
%% turned into a single hyperlink (to the DOI, or the URL if no DOI is
%% given) -- APS style.  \fullcite (the chapter openers) is a citation, not
%% a bibitem, so the hooks below never touch it: it keeps showing the DOI as
%% "DOI: <link>" text exactly as before.
\newtoggle{bbx@biblink}                 % true only inside the reference list
\AtEveryBibitem{%
  \toggletrue{bbx@biblink}%
  \ifentrytype{article}{}{\clearfield{doi}}}% non-articles: drop DOI as before
\AtEveryCite{\togglefalse{bbx@biblink}}
\DeclareFieldFormat{doi}{%              % used by \fullcite (DOI shown as text)
  DOI:\addspace
  \ifhyperref
...
%% "journal volume, pages (year)" run used as the hyperlink text
...
%% wrap that run in an \href to the DOI (else the URL); plain text if neither
\newbibmacro*{nat@jouryear+link}{%
  \iffieldundef{doi}
    {\iffieldundef{url}
       {\usebibmacro{nat@jouryear}}
       {\href{\thefield{url}}{\usebibmacro{nat@jouryear}}}}
    {\href{https://doi.org/\thefield{doi}}{\usebibmacro{nat@jouryear}}}}
